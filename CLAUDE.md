...
1. `inbox/` durchgehen. Jeder Unterordner = ein Ticket:
   - `ticket.md` mit Frontmatter (`title`, `type`, `created`, `status`) + Beschreibung
   - `shot-*.png|jpg|…` = die zugehörigen Screenshots
2. Die Bilder **lesen** (Read-Tool kann PNG/JPG) und in die Ticket-Erstellung einbeziehen.
